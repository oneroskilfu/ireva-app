 ...
 * Insights API Routes
 * 
 * Handles all insight and reporting endpoints for investment analytics,
 * ROI tracking, and investor dashboard data
 */
...
const { users, investments, properties, transactions } = require('../../shared/schema');
const NotificationService = require('../services/notifications/NotificationService');
const nodemailer = require('nodemailer');

// Get ROI history data
router.get('/insights/roi-history', authenticateUser, async (req, res) => {
  try {
...
router.get('/insights/wallet-activity', authenticateUser, async (req, res) => {
  try {
    const { timeframe = 'last_6_months' } = req.query;
    const userId = req.user.id;
...
router.get('/insights/project-participation', authenticateUser, async (req, res) => {
  try {
    const userId = req.user.id;
...
router.get('/insights/export-csv', authenticateUser, async (req, res) => {
  try {
    const { timeframe = 'last_6_months' } = req.query;
    const userId = req.user.id;
...
router.post('/insights/email-report', authenticateUser, async (req, res) => {
  try {
...
router.post('/insights/schedule-report', authenticateUser, async (req, res) => {
  try {
...
router.get('/insights/performance-metrics', authenticateUser, async (req, res) => {
  try {
    const userId = req.user.id;
