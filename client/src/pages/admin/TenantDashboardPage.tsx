...
import { ArrowLeft, Building, BarChart3, Settings, Users, Mail } from 'lucide-react';
import { Badge } from '@/components/ui/badge';

const TenantDashboardPage: React.FC = () => {
  const [, setLocation] = useLocation();
  const { user, isLoading: authLoading } = useAuth();
  const [match, params] = useRoute('/admin/tenants/:id/dashboard');
  ...
  useEffect(() => {
    ...
    document.title = tenantDetails?.data?.name 
      ? `${tenantDetails.data.name} Dashboard | iREVA Admin` 
      : 'Tenant Dashboard | iREVA Admin';
  }, [tenantDetails]);
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between mb-8 gap-4">
        <div>
          ...
          <p className="text-muted-foreground mt-1 flex items-center">
            ...
            Analytics and performance metrics for this organization
          </p>
        </div>
        ...
        <div className="flex flex-wrap gap-2">
          ...
          <Button 
            variant="outline" 
            size="sm"
            onClick={() => setLocation(`/admin/tenants/${tenantId}/invitations`)}
          >
          ...
          <Button 
            variant="outline" 
            size="sm"
            onClick={() => setLocation(`/admin/tenants/${tenantId}/settings`)}
          >
